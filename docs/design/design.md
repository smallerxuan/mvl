...
![MVVM-Lite 分层架构：后台任务/ISR 经解耦层（Model 状态中心 + mvl_evt 事件总线）与 LVGL 主任务内的 ViewModel、View 接口层、View 交互，①~⑤ 标注双向数据流](../assets/design_mvvm_layers.png)

<!-- 图源：docs/assets/design_mvvm_layers.mmd（mermaid），修改后重新渲染 PNG -->
